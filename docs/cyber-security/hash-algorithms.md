...
📍مثال هایی از الگوریتم های رمزنگاری متقارن: AES, ChaCha20, DES, 3DES, Blowfish, Twofish, Serpent, RC4, Camellia, Salsa20, IDEA, CAST5, Skipjack, Kuznyechik, Safer, Threefish, etc
...
یک عدد غیرقابل پیشبینی (معمولاً بزرگ و تصادفی) برای شروع تولید یک جفت کلید قابل قبول و مناسب برای استفاده توسط یک الگوریتم کلید نامتقارن استفاده می شود.
...
در این مثال، پیام با کلید خصوصی آلیس به صورت دیجیتالی امضا شده است، اما خود پیام رمزگذاری نشده است. 1) آلیس پیامی را با کلید خصوصی خود امضا می کند. 2) باب با استفاده از کلید عمومی آلیس می تواند تأیید کند که آلیس پیام را ارسال کرده و پیام تغییر نکرده است.
...
همچین عکس هایی مد نظر است❗

اما اصلا چرا باید این همه کار را سخت و پیچیده کنیم؟! بدون دلیل نیست، فرض کنید شما یک فرمانده جنگی هستی و می خواهی یک "فایل دستور محرمانه" را به یک ژنرال در جبهه دیگر بفرستی، در ابتدا از رمزنگاری متقارن استفاده می کنی مثالا کد 1234 که بایستی به طریقی به ان ژنرال برسد (طبیعتا کد هر فایل دستور عملیات هر دفعه عوض می شود) بنابراین نمی شود هر دو فرد ان را حفظ کنند، اما یک مشکل اساسی دیگر هم داری! چطور ان کد را به ژنرال برسانی در حالی که مطمئن باشی جاسوس دشمن ان را در مسیر نمی دزدد؟ اگر کد را با یک کبوتر می فرستی، جاسوس ممکن است کبوتر را بزند و کد را بردارد. اگر با تلفن می گویی، ممکن است خط شنود شود و ... اما اگر از رمزنگاری نامتقارن استفاده شود اصلا نیازی به فرستادن رمز نیست! چه طور؟ فایل دستورات عملیات ها را با رمز عمومی 1234 رمزنگاری می شوند حالا ان ژنرال که کلید خصوصی مختص به خود را دارد مثالا 4321 می تواند ان فایل دستورات عملیات را رمزگشایی کند و بخواند.

📍مثال هایی از الگوریتم های رمزنگاری نامتقارن: RSA, Diffie-Hellman, ECC, DSA, EIgamal, Paillier, Cramer-Shoup, PAKE, YAK, NTRUEncrypt, McEliece, Merkle-Hellman Knapsack, etc

### تفاوت symmetric cryptography با asymmetric cryptography
تفاوت این دو شیوه رمزنگاری را می توان به صورت زیر خلاصه نمود:
...
graph TD
    ...
    B[(f: hash function)]
    C[out: fixed-length binary digest]
...
یعنی مهم نیست ورودی چقدر بزرگ یا کوچک باشد در نهایت خروجی تابع هش همیشه یک طول ثابت دارد (مثلا یک رشته 128 یا 256 بیتی از 0 و 1) البته در برخی از الگوریتم های هش، طول خروجی می تواند توسط کاربر مشخص شود.

```mermaid
graph TD
    %% defining styles
    classDef greyBox fill:#E2ECE9, color:#000000
    classDef pinkBox fill:#E6C0C0, color:#000000
    classDef greenBox fill:#00DD00, color:#000000
    classDef hashBox fill:#404040, color:#FFFFFF

    %% input raw data
    subgraph Data ["ورودی الگوریتم هش (input data)"]
        style Data fill:none,stroke:none,font-weight:bold
        RawData1["<b>فایل متنی (Text File)</b><br/>Document.txt<br/>10100011001010111<br/>00110100110010100<br/>000101001010111<br/>10010110100011001"]:::greenBox
        RawData2["<b>فایل فشرده (ZIP Archive)</b><br/>Backup.zip<br/>101000110010101110101010101<br/>10010101100100110100110010100<br/>00101001010100010101001010111<br/>10010100101000101010100010100"]:::pinkBox
        RawData3["<b>ویدیو (Video File)</b><br/>Video.mp4<br/>10100011001010111010101010100101001<br/>1001010110010011010011001010001001000<br/>0010100101010001010100101011100001101<br/>1001010010100010101010001010010100100"]:::greyBox
    end

    HashAlgorithm["Hash Algorithm<br/>(MD5 / SHA-256, etc)"]:::hashBox

    %% output hash value
    subgraph HashValues ["خروجی الگوریتم هش (hash value)"]
        style HashValues fill:none,stroke:none,font-weight:bold
        Data1HashValue["<b>هش ویدیو</b><br/>00101101101100101011001010"]:::greyBox
        Data2HashValue["<b>هش فایل زیپ</b><br/>10100100101100101011010011"]:::pinkBox
        Data3HashValue["<b>هش فایل متنی</b><br/>10001101001010010101001001"]:::greenBox
    end

    RawData1 --> HashAlgorithm
    RawData2 --> HashAlgorithm
    RawData3 --> HashAlgorithm

    HashAlgorithm --> Data1HashValue
    HashAlgorithm --> Data2HashValue
    HashAlgorithm --> Data3HashValue
```
...
📌 اگر در سیستم عامل ویندوز هستید به رابط خط فرمان ویندوز که CMD باشد وارد شوید، و طبق شیوه اموزش داده شده، کد ها را وارد کنید.
...
📌اگر در سیستم عامل لینوکس هستید به رابط خط فرمان لینوکس که shell باشد وارد شوید، و طبق شیوه اموزش داده شده، کد ها را وارد کنید.
...
📌اگر در سیستم عامل مک هستید به رابط خط فرمان مک که Terminal باشد وارد شوید، و طبق شیوه اموزش داده شده، کد ها را وارد کنید.
...
•الگوریتم هش BCrypt که بنده نیز در پروژه جاوا خود از ان استفاده کرده بودم، مبتنی بر یک الگوریتم قدیمی تر به نام blowfish است، طول خروجی ان 60 کارکتر است و سختی محاسباتی (هزینه محاسبه) ان می تواند با یک عدد صحیح که نشان دهند تکرار الگوریتم است مشخص شود. (باقی این موارد هم پارامتر هایی و ویژگی هایی برای تنظیم الگوریتم هش دارند)
...
•الگوریتم هش PBKDF2 الگوریتمی سنتی مبتنی بر تکرار الگوریتم هش SHA است (یعنی داده های را چند بار به صورت متوالی هش می کند)، اگرچه قدیمی است اما همچنان در شرکت ها و سازمان ها استفاده می شود و امنیت مناسب و قابل قبولی هم دارد.
